...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize):
        ...
        # For the RGB image: resize, convert to Tensor, normalize
        ...
        # For the mask: resize with NEAREST (so it stays 0/1), then to tensor
        self.gt_transform = transforms.Compose([
            transforms.Resize((self.testsize, self.testsize), interpolation=Image.NEAREST),
            transforms.ToTensor()  # this will give us a float tensor in [0.0 or 1.0]
        ])
    ...
    def load_data(self):
        ...
        image = self.transform(image)  # shape => [3, testsize, testsize]
        image = image.unsqueeze(0)     # shape => [1, 3, H, W] if your model expects a batch
        
        # 3) Load the mask
        gt = self.binary_loader(self.gts[self.index])
        # 4) Apply the mask transform, including resize
        gt = self.gt_transform(gt)     # shape => [1, testsize, testsize]
        
        # 5) Create a name for saving
