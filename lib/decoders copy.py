...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.atrous_block1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, dilation=1, bias=False)
...
class GCASCADE_bl(nn.Module):
    def __init__(self, channels=[512, 320, 128, 64], drop_path_rate=0.0, img_size=224, k=11, padding=5, conv='mr', gcb_act='gelu', activation='relu'):
        super(GCASCADE_bl, self).__init__()
        ...
        # ASPP for multi-scale feature fusion
        ...
        # Spatial Attention (SPA)
        self.spa = SPA()

    def forward(self, x, skips):
        # Apply ASPP before the first GCAM
        x = self.aspp(x)
