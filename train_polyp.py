...
def structure_loss(pred, mask, boundary_weight=0.5, boundary_kernels=[3, 5, 7], gamma=2.0):
    """
    Enhanced structure loss with focal loss and multi-scale boundary detection
    Args:
        pred: predicted output (logits)
        mask: ground truth mask (values 0-1)
        boundary_weight: weight for boundary loss component (default: 0.5)
        boundary_kernels: list of kernel sizes for multi-scale boundary detection
    """
    ...
    main_loss = (wbce + wiou).mean()

    # --- Multi-scale Boundary Detection ---
    with torch.no_grad():
        boundary_masks = []
        for k_size in boundary_kernels:
            dilated = F.max_pool2d(mask, kernel_size=k_size, stride=1, padding=k_size//2)
            eroded = -F.max_pool2d(-mask, kernel_size=k_size, stride=1, padding=k_size//2)
            boundary_mask = (dilated - eroded).abs()
            boundary_mask[boundary_mask >= 0.5] = 1.0
            boundary_mask[boundary_mask < 0.5] = 0.0
            boundary_masks.append(boundary_mask)
        

        if len(boundary_masks) > 1:
            boundary_mask = torch.max(torch.stack(boundary_masks, dim=0), dim=0)[0]
        else:
    ...
    focal_weight = (1 - pt) ** gamma  # lower probability -> higher weight
    
    # Apply focal weighting to boundary BCE
    boundary_focal_loss = (focal_weight * bce * boundary_mask).sum() / (boundary_mask.sum() + 1e-8)
    
    # Combine losses
    total_loss = main_loss + boundary_weight * boundary_focal_loss
...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        res = F.upsample(res1 + res2 + res3 + res4, size=gt.shape[-2:], mode='bilinear', align_corners=False) # additive aggregation and upsampling
...
def train(train_loader, model, optimizer, epoch, test_path, model_name = 'MSGBANET'):
    model.train()
    global best
    ...
    if (epoch + 1) % 1 == 0:
    	...
    	dict_plot['ETIS-LaribPolypDB'].append(meandice)
...
if __name__ == '__main__':
    dict_plot = {'ETIS-LaribPolypDB':[]}
    name = ['ETIS-LaribPolypDB']
    ##################model_name#############################
    model_name = 'Polyp_MSGBANET_img_size352bs4_Run1'
